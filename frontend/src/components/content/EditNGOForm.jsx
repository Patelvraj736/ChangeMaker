...
const EditNGOForm = ({ ngoId, onNGOUpdated }) => {
    ...
    useEffect(() => {
        const fetchNGODetails = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/ngos/edit/${ngoId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`${import.meta.env.VITE_API_URL}/api/ngos/${ngoId}`, formData, {
                headers: {
